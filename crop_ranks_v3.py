...
for row in range(len(y_starts)):
    for col in range(len(x_starts)):
        ...
        # FIRST: chop bottom 25% to remove text (BRONZE, GOLD, ACE, etc.)
        # Keep only top 75% which contains the logo
        logo_height = int(ch * 0.75)
        no_text = cell.crop((0, 0, cw, logo_height))
        
        # SECOND: chop top edge if there's red/orange text residue there
        # The "100/100" text is in top-left corner, small. Trim top 8% to be safe.
        top_trim = int(logo_height * 0.08)
        no_top_text = no_text.crop((0, top_trim, cw, logo_height))
        
        # THIRD: find bounding box of non-black content in the remaining area
