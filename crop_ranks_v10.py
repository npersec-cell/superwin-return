...
# Trim margins to remove grid lines that are INSIDE cell boundaries
MARGIN = 3
...
for row in range(len(y_starts)):
    for col in range(len(x_starts)):
        ...
        # Trim margins to remove grid lines inside cell
        cell = img.crop((x0 + MARGIN, y0 + MARGIN, x1 + 1 - MARGIN, y1 + 1 - MARGIN))
        cw, ch = cell.size

        # Remove top 15% (small red/orange text area)
        y_top = int(ch * 0.15)
        # Remove bottom 25% (rank name text)
        y_bottom = int(ch * 0.75)

        logo_area = cell.crop((0, y_top, cw, y_bottom))

        # Clean up any remaining orange/yellow grid pixels
        ...
        # Make 1:1 canvas with pure black background + 10% padding
        lw, lh = logo_clean.size
        sq = max(lw, lh)
        ...
        # Shift down slightly for visual centering (logos have top-heavy elements like stars/wings)
        offset_y = (canvas_size - lh) // 2 + int(canvas_size * 0.03)

        final.paste(logo_clean, (offset_x, offset_y), logo_clean)

        out_path = os.path.join(out_dir, f"{name}.png")
        final.save(out_path)
        print(f"Saved {name}: logo={lw}x{lh}, canvas={canvas_size}x{canvas_size}, offset=({offset_x},{offset_y})")
