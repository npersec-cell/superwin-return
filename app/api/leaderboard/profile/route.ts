...
export async function GET(request: NextRequest) {
  try {
    ...
    const allUserStats = new Map<string, {
      profitScore: number;
    ...
    for (const entry of (entries || [])) {
      const stat = allUserStats.get(entry.user_id);
      if (stat) {
    ...
    for (const [uid, stat] of allUserStats) {
    ...
    const leaderboardData = Array.from(allUserStats.entries()).map(([uid, stats]) => {
      const user = users?.find(u => u.id === uid);
      return {
        userId: uid,
    ...
    // ── Calculate Overall score for ALL users ──
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        // Rank data (calculated independently)
        rank: mostOrangeAmmoRank,
        rankPercentile: mostOrangeAmmoRank / totalUsers,
        rankName: rankInfo.name,
        rankIcon: rankInfo.icon,
        totalUsers,
        // Leaderboard ranks (calculated independently)
