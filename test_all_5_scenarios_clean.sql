...
BEGIN
  ...
  IF v_admin_id IS NULL THEN
    RAISE EXCEPTION 'ERROR: ไม่พบ Admin user';
  END IF;
...
BEGIN
  ...
  IF (v_after->>'coin_balance')::NUMERIC > (v_before->>'coin_balance')::NUMERIC - 100 THEN
    RAISE NOTICE '   ✅ สถานการณ์ที่ 3 ผ่าน: ได้คืนประกัน 50 เปอร์เซ็นต์ แล้ว';
  ELSE
...
BEGIN
  ...
  -- จำลองการ Refund (อัพเดท prediction เป็น canceled)
...
-- ============================================
-- คำเตือน: Uncomment บรรทัดด้านล่างเพื่อลบข้อมูลทดสอบ
--
-- DELETE FROM coin_ledger WHERE user_id = (SELECT id FROM users WHERE email = 'test_automated@example.com');
-- DELETE FROM prediction_entries WHERE prediction_id IN (SELECT id FROM predictions WHERE question LIKE 'ทดสอบที่%');
