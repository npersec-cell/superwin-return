-- Migration: Fix lifetime_profit clamp + protect profit_score from going negative
-- Date: 2026-06-10
-- Purpose: Allow lifetime_profit to go negative (real P&L),
--          and ensure profit_score (green ammo) never goes below 0

-- ================================================================
-- FIX 1: Recreate place_prediction_atomic without GREATEST(0) on lifetime_profit
--          and ADD GREATEST(0) protection on profit_score
-- ================================================================
...
BEGIN
  ...
  IF p_insurance THEN
    v_rate := GREATEST(0.05, 0.20 - ((LN(GREATEST(p_amount, 10)) / LN(10) - 1) * 0.05));
    v_insurance_cost := GREATEST(FLOOR(p_amount * v_rate), 1);
  ...
  -- FIX: profit_score protected at 0 (green ammo never goes negative)
  v_profit_score_after := CASE 
    WHEN p_insurance THEN GREATEST(0, v_user.profit_score - v_insurance_cost)
    ELSE v_user.profit_score
  END;
  
  -- FIX: lifetime_profit CAN go negative (real P&L tracking)
  v_lifetime_profit_after := v_user.lifetime_profit - p_amount;
...
-- ================================================================
-- FIX 2: Add CHECK constraint to prevent profit_score from going negative
-- ================================================================

DO $$
BEGIN
  -- Remove old constraint if exists (with unknown name)
  ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_profit_score_non_negative;

  -- Add new constraint
  ALTER TABLE public.users
  ADD CONSTRAINT users_profit_score_non_negative CHECK (profit_score >= 0);
EXCEPTION WHEN others THEN
  -- Constraint might already exist or data violates it
  RAISE WARNING 'Could not add profit_score check constraint: %', SQLERRM;
END $$;


-- ================================================================
-- FIX 3: Update existing users.profit_score to be non-negative (safety)
-- ================================================================

UPDATE public.users
SET profit_score = GREATEST(0, profit_score)
