-- Fix balance_after in refund_prediction_atomic()
-- Date: 2026-06-07
...
BEGIN
  ...
  LOOP
    ...
    PERFORM 1 FROM users WHERE id = v_entry.user_id FOR UPDATE;

    -- Update user balances (let DB calculate)
    UPDATE users
    SET
      coin_balance = coin_balance + v_entry.amount,
      profit_score = profit_score + (CASE WHEN v_entry.insurance THEN v_entry.insurance_cost ELSE 0 END),
      lifetime_profit = GREATEST(0, lifetime_profit + v_entry.amount),
      updated_at = p_refunded_at
    WHERE id = v_entry.user_id;

    -- ✅ READ ACTUAL BALANCES FROM DB (not calculated)
    ...
    IF v_entry.insurance AND v_entry.insurance_cost > 0 THEN
      ...
      VALUES (
        ...
        v_user_profit_after,  -- ✅ Actual DB value
        'prediction_entry',
        v_entry.id,
