-- =====================================================
-- Migration: Remove unique constraint to allow free multi-predict
-- Date: 2026-07-22
-- PURPOSE: Allow users to predict multiple times on the same question
--          (both different options and same option)
-- =====================================================

-- ── 1. DROP UNIQUE CONSTRAINT: อนุญาตให้ทายคำถามเดิมซ้ำได้ ──
ALTER TABLE prediction_entries
  DROP CONSTRAINT IF EXISTS uniq_user_prediction;

-- ── 2. UPDATE RPC FUNCTION: ลบการเช็ค duplicate prediction ──
...
BEGIN
  ...
  -- ========== 6. [REMOVED] Duplicate prediction check — now allows multi-predict ==========
