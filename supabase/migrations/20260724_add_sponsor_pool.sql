-- ============================================================
-- Migration: Add sponsor_pool ("กระสุมส้ม") to predictions
-- Every question gets auto bonus pool to incentivize participation
-- ============================================================

-- Step 1: Add sponsor_pool column
ALTER TABLE predictions
  ADD COLUMN IF NOT EXISTS sponsor_pool INTEGER DEFAULT 500;

-- Set existing questions to have 500 coins sponsor pool
UPDATE predictions SET sponsor_pool = 500 WHERE sponsor_pool IS NULL;

-- Step 2: Update resolve_prediction_atomic to include sponsor_pool in distributable pool
...
BEGIN
  -- Lock prediction row
  SELECT id, tournament_name, question, fee_rate, status, sponsor_pool
  INTO v_prediction
  FROM predictions
  ...
  v_sponsor_pool := COALESCE(v_prediction.sponsor_pool, 0);

  -- Calculate total pool (user bets only — sponsor added later)
  ...
  -- 🍊 กระสุมส้ม: รวม sponsor_pool ในพูลทัง้หมดก่อนหาค่าธรรมเนียม
  v_distributable := FLOOR(((v_total_pool + v_sponsor_pool) * (1 - v_fee_rate))::numeric)::integer;
