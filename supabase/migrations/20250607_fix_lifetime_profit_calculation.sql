-- Migration: Fix lifetime_profit calculation in resolve_prediction_atomic
-- Date: 2026-06-07
-- FIX: lifetime_profit should only add net profit, not full payout
-- (principal was already deducted when bet was placed)
...
BEGIN
  ...
  LOOP
    ...
    SET
      ...
      -- FIXED: Only add net profit (principal was already deducted when bet was placed)
  ...
  LOOP
    ...
    IF v_entry.insurance THEN
      ...
      IF v_insurance_refund > 0 THEN
        ...
        -- Insurance refund: return 50% to coin_balance
        -- For lifetime_profit: this is a partial recovery, so we add it back
        UPDATE users
        SET
          coin_balance = coin_balance + v_insurance_refund,
          -- Partial recovery from loss
