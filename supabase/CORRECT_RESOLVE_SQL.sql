-- ============================================================
-- FIXED SQL: Atomic Resolve Function (all syntax errors fixed)
-- Copy ENTIRE script below → Paste in Supabase SQL Editor → Click "Run"
-- ============================================================

-- Step 1: Add insurance column (safe if already exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 1b: Add insurance_cost column (safe if already exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Step 2: Update coin_ledger type constraint
...
-- Step 3: Drop old function (both signatures to avoid conflicts)
DROP FUNCTION IF EXISTS resolve_prediction_atomic(uuid, uuid, timestamptz);
DROP FUNCTION IF EXISTS resolve_prediction_atomic(text, text, timestamptz);

-- Step 4: Create the FIXED function (syntax verified)
...
SELECT 'SUCCESS: Function created without syntax errors!' AS result;
