-- Fix: real-time lifetime_profit calculation
-- Run this in Supabase SQL Editor AFTER pulling the latest code
-- Date: 2026-06-07

-- The function already exists, we just need to update the lifetime_profit logic:
-- 1. On win: ADD FULL payout (amount was already deducted on bet)
-- 2. On insurance refund (loss): ADD insurance_refund to lifetime_profit

-- Drop and recreate with correct lifetime_profit handling
...
BEGIN
  ...
  LOOP
    ...
    SET
      ...
      -- Real-time profit: amount was already deducted on bet, add FULL payout here
    ...
    IF v_entry.insurance THEN
      ...
      IF v_insurance_refund > 0 THEN
        ...
        SET
          ...
          -- Partial recovery: insurance refund adds back to lifetime_profit
...
SELECT 'Function updated with real-time lifetime_profit!' AS result;
