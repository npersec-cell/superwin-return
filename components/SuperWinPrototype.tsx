...
export default function SuperWinPrototype() {
  ...
  // ── Tab Navigation ──
  const [activeTab, setActiveTab] = useState<"prediction" | "btc">("prediction");
  ...
  async function loadSettings() {
    ...
    if (response.ok && payload.ok && payload.data) {
      // Check localStorage: did user dismiss THIS specific announcement?
      const dismissedText = (() => { try { return localStorage.getItem("superwin_announcement_dismissed_text") || ""; } catch { return ""; } })();
      const currentAnnouncement = payload.data.announcement || "";
      const wasDismissed = dismissedText.length > 0 && dismissedText === currentAnnouncement;
      
      setShowAnnouncement(!wasDismissed);
  ...
  function renderProbabilityChart(question: Question) {
    ...
    // Only show options with non-zero percentage (max 4)
    const top4 = withPct.filter(o => o.pct > 0).slice(0, 4);
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                ...
                try { localStorage.setItem("superwin_announcement_dismissed_text", settings.announcement || ""); } catch { /* ignore */ }
              }}
              style={{
                position: "absolute",
                top: "6px",
        ...
        <nav style={{ 
          ...
          <button
            ...
            style={{
              ...
              background: "transparent",
              border: "none",
              borderBottom: activeTab === "prediction" ? "2px solid var(--yellow)" : "2px solid transparent",
              color: activeTab === "prediction" ? "var(--yellow)" : "var(--text-weak)",
              fontWeight: activeTab === "prediction" ? "700" : "500",
              fontSize: "13px",
              cursor: "pointer",
              transition: "all 0.2s",
              display: "flex",
          ...
          <button
            ...
            style={{
              ...
              background: "transparent",
              border: "none",
              borderBottom: activeTab === "btc" ? "2px solid var(--yellow)" : "2px solid transparent",
              color: activeTab === "btc" ? "var(--yellow)" : "var(--text-weak)",
              fontWeight: activeTab === "btc" ? "700" : "500",
              fontSize: "13px",
              cursor: "pointer",
              transition: "all 0.2s",
              display: "flex",
